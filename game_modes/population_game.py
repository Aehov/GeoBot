...
def play_population(message, bot: telebot, user: dict) -> None:
    ...
    bot.send_message(message.chat.id, "В какой стране больше населения?")
...
def check_answer(message, bot: telebot, user: dict, c1: dict, c2: dict) -> None:
    ...
    if 'timer' not in user:
        ...
        user['hits'] = 0
        user['index'].clear()
        bot.send_message(message.chat.id, "Выход из режима «Больше-меньше»", reply_markup = ReplyKeyboardRemove())
        time.sleep(1)
        bot.send_message(message.chat.id, "Выберите новый игровой режим из меню.")
        return
    
    higher = c1 if c1['population'] > c2['population'] else c2
    attemp = c2['country'] if message.text == "Больше" else c1['country']
    ...
    if(higher['country'] == attemp):
        ...
        user['timer'].cancel()
        bot.send_chat_action(message.chat.id, "typing")
        bot.send_message(message.chat.id, f"<b><i>НЕПРАВИЛЬНО</i></b>, {c2['country']} {c2['flag'].encode().decode('unicode_escape')} население {c2['population']: ,}",parse_mode="html", disable_web_page_preview=True, reply_markup=ReplyKeyboardRemove())
        bot.send_chat_action(message.chat.id, "typing")
        time.sleep(1)
        ...
        time.sleep(1)
        bot.send_message(message.chat.id, "Нажми /population чтоб играть сначала")
...
def next_question(message, bot: telebot, user: dict) -> None:
    ...
    markup = ReplyKeyboardMarkup(
        ...
        input_field_placeholder= "Выбери один",
