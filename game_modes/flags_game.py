...
def check_answer(message, bot: telebot, user: dict, country: str) -> None:
    ...
    if 'timer' not in user:
        ...
        user['hits'] = 0
        user['index'].clear()
        bot.send_message(message.chat.id, "Выход из режима конкурса «Флаги»", reply_markup = ReplyKeyboardRemove())
        time.sleep(1)
        ...
        user['timer'].cancel()
        bot.send_chat_action(message.chat.id, "typing")
        bot.send_message(message.chat.id, f"<b><i>НЕПРАВИЛЬНО</i></b>, правильный ответ <b><i>{country}</i></b>",parse_mode="html", disable_web_page_preview=True, reply_markup=ReplyKeyboardRemove())
        time.sleep(1)
        bot.send_chat_action(message.chat.id, "typing")
        ...
        time.sleep(1)
        bot.send_message(message.chat.id, "Нажми /flags чтоб начать сначала")
...
def handle_timeout(message, bot: telebot, user: dict):
    ...
    user['hits'] = 0
    user['index'].clear()
    bot.send_message(message.chat.id, "Нажми /flags чтоб играть сначала")